...
import { DIRECT_CHANNELS, contactData } from '@/data/contactData';
...
export const TransmissionControlPanel: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div id="status" className="scroll-mt-28">
        <StatusWidget />
      </div>
      ...
      <div id="channels" className="p-5 sm:p-6 bg-[#0b0e14]/90 backdrop-blur border border-slate-800 rounded-xl shadow-lg space-y-4 font-mono scroll-mt-28">
        <div className="text-xs text-slate-400 tracking-wider mb-2">
          {contactData.controlPanel.directChannelsTitle}
        </div>
        ...
        <div className="space-y-2.5">
          {DIRECT_CHANNELS.map((ch) => (
            <ChannelRow
              ...
              badgeVariant={ch.badgeVariant}
              iconSvg={getIcon(ch.name)}
            />
          ))}
        </div>
      </div>
